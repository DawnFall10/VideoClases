 ...
 */
 function Question(title, choices) {
        ...
        self.choices = ko.observableArray(ko.utils.arrayMap(choices, function(item) {
...
function viewModel() {
    ...
    self.editTestBoolean = ko.observable(false);

    self.editTest = function () {
     self.editTestBoolean(true);
        ...
        $.when($.ajax("/teacher/download-homeworks/" + value + "/")).done(
            function (result) {
                ...
                for (i = 0; i < result.homeworks.length; i++) {
                    ...
                    if(!a.has_pq || a.id == original_homework){
    ...
    self.addQuestion = function () {
        this.questions.push(new Question("", ["", "","","no se"]));
    };
    ...
    self.submitForms = function() {
        ...
        if ($("#new-homework-form").valid() && self.check_time(true)) {
            $(".loader").fadeIn("slow");
            self.submitNewHomeworkForm();
        }
    }
...
$(document).ready(function() {
    $('#new-homework-form').validate({ // initialize the plugin
        messages: {
            course: {
