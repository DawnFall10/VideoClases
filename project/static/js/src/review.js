...
function ViewModel() {
    ...
    self.clickNextVideoclase = function() {
        ...
        if($("#answerForm").valid()){
            ...
            self.loading(true);
            self.submitStudentEvaluation();
            self.submitStudentResponse();
        }

    };

    self.review = function(value) {
        self.responseValues.value(value);
    };

    self.submitStudentEvaluation = function(data, event) {
        var fd = new FormData();
        ...
        if(self.responseValues.criteria && self.responseValues.criteria()){
            ...
            for(let c of self.responseValues.criteria()){
                criteriaResponse.push({ value: c.response(), criterion: c.id });
            }
        ...
        return $.ajax('/student/review-video/', {
            data: fd,
            type: "post",
            processData: false,
            contentType: false,
    ...
    self.submitStudentResponse = function() {
        var fd = new FormData();
        ...
        return $.ajax('/student/review-videoclase-form/', {
            data: fd,
            type: "post",
            processData: false,
            contentType: false,
